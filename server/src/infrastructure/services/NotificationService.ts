  ...
  async sendPasswordResetOtpEmail(params: {
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
        <div style="background: #0a0a0f; padding: 24px; text-align: center;">
          <h1 style="color: #ccff00; margin: 0; font-size: 24px; font-weight: 800; letter-spacing: 1px;">NOXPHERE</h1>
          <p style="color: #94a3b8; margin: 4px 0 0 0; font-size: 12px; text-transform: uppercase;">Password Reset Request</p>
        </div>
        <div style="padding: 30px; color: #1e293b;">
          <h2 style="font-size: 18px; font-weight: 700; margin-top: 0;">Reset Your Password</h2>
          <p style="font-size: 14px; line-height: 1.6; color: #475569;">
            ${params.recipientName ? `Hello <strong>${params.recipientName}</strong>,<br/><br/>` : ''}
            We received a request to reset the password for your Noxphere account. Use the verification code below to set a new password:
          </p>
          ...
          <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 14px 18px; margin: 20px 0;">
            <p style="margin: 0; font-size: 13px; color: #475569; line-height: 1.5;">
              <strong>Security Notice:</strong> If you did not request a password reset, you can safely ignore this email. Your current password remains unchanged.
            </p>
          </div>
    ...
    logger.info(`[NotificationService] Sending password reset OTP to ${params.to}`);

    return await this.dispatchMail({
      to: params.to,
      subject: `${params.otp} is your Noxphere Password Reset Code`,
      html,
    });
  }
}
