...
const statusConfig: Record<
  string,
  { label: string; variant: "green" | "blue" | "yellow" | "red" | "gray" }
> = {
...
const STATUS_ACTIONS: SelectionCandidate["status"][] = [
  "selected",
  "shortlisted",
  "on_hold",
  "not_selected",
...
const SelectionPage: React.FC = () => {
  ...
  const canBroadcast = useSelector(
    (s: RootState) => s.auth.user?.permissions?.canSendNotifications === true,
  );
  ...
  const handleUpdate = async (
    id: string,
    status: SelectionCandidate["status"],
  ) => {
    try {
      await updateStatus({
        id,
  ...
  const handleNotifyAll = async () => {
    ...
    try {
      ...
      toast.success(
        `Notified ${res.notified} guardian${res.notified === 1 ? "" : "s"}`,
      );
    } catch {
  ...
  const counts = {
    ...
    not_selected: list.filter(
      (s) => s.status === "not_selected" || s.status === "released",
    ).length,
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {list.length} players in evaluation
          </p>
        </div>
          ...
          <button
            ...
            className={clsx(
              ...
              activePhase === i
                ...
                : "bg-transparent border-slate-300 dark:border-white/10 text-slate-500 hover:text-slate-900 dark:hover:text-white hover:border-slate-400 dark:hover:border-white/20",
            )}
          >
            <span
              className={clsx(
                "w-4 h-4 rounded-full border text-2xs flex items-center justify-center font-900",
                activePhase === i
        ...
        {[
          {
            ...
            border: "border-field-400/20",
          },
          {
            key: "shortlisted",
            label: "Shortlisted",
            color: "text-ice-400",
            border: "border-ice-400/20",
          },
          {
            key: "on_hold",
            label: "On Hold",
          ...
          <div
            key={s.key}
            className={clsx("card p-3 text-center border", s.border)}
          >
            <p
              className={clsx(
                "font-display font-900 text-xl sm:text-2xl",
                s.color,
              )}
            >
      ...
      {isError && (
        <EmptyState
          title="Couldn't load the selection list"
          ...
          <table className="w-full min-w-[540px]">
            <thead>
              <tr className="border-b border-white/5 bg-pitch-700/30">
                ...
                <th className="text-center px-5 py-3 section-title hidden md:table-cell">
                  Coach Vote
                </th>
                <th className="text-left px-5 py-3 section-title hidden lg:table-cell">
                  Coach Note
                </th>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-center">
                      <span className="font-display font-extrabold text-volt-400 text-lg">
                        {player.rating}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-center hidden md:table-cell">
                      <span
                        className={clsx(
                          ...
                          player.coachVote === "Recommended"
                            ? "text-field-400"
                            : player.coachVote === "Not Recommended"
                              ? "text-ember-400"
                              : "text-volt-400",
                        )}
                      >
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              <p className="font-display font-bold text-slate-900 dark:text-white uppercase">
                Ready to Finalize Selection?
              </p>
              <p className="text-xs text-slate-500 mt-1">
                {counts.selected} selected · {counts.shortlisted} shortlisted ·{" "}
                {counts.not_selected} released
              </p>
            </div>
            {canBroadcast && (
          ...
          <div className="relative card shadow-panel w-full max-w-md animate-slide-up">
            <div className="flex items-center justify-between p-5 border-b border-white/5">
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <p className="font-display font-bold text-slate-900 dark:text-white text-lg uppercase">
                    {selectedPlayer.name}
                  </p>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="card p-3 text-center">
                  <p
                    className={clsx(
                      "text-sm font-bold",
                      selectedPlayer.coachVote === "Recommended"
                        ? "text-field-400"
                        : selectedPlayer.coachVote === "Not Recommended"
                          ? "text-ember-400"
                          : "text-volt-400",
                    )}
                  >
                    {selectedPlayer.coachVote}
                  </p>
                  <p className="text-2xs text-slate-500 mt-0.5 uppercase tracking-wide">
                    Coach Vote
                  </p>
                  ...
                  {STATUS_ACTIONS.map((s) => {
                    ...
                    return (
                      <button
                        key={s}
                        onClick={() => {
                          handleUpdate(selectedPlayer.id, s);
                        ...
                        className={clsx(
                          ...
                          isActive
                            ...
                            : "border-slate-300 dark:border-white/10 text-slate-500 hover:text-slate-900 dark:hover:text-white hover:border-slate-400 dark:hover:border-white/20",
                        )}
                      >
                        {cfg.label}
                      </button>
