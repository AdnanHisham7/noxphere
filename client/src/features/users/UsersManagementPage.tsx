      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
            {isLoading
              ? "Loading…"
              : `${data?.total ?? 0} accounts across all academies`}
          </p>
        </div>
        <Button
          icon={<Plus size={16} />}
          ...
          <table className="w-full min-w-[540px]">
            <thead>
              <tr className="border-b border-white/5 bg-pitch-700/30">
                ...
                <th className="text-left px-5 py-3 section-title hidden sm:table-cell">
                  Role
                </th>
                <th className="text-left px-5 py-3 section-title hidden md:table-cell">
                  Phone
                </th>
                  ...
                  <td className="px-5 py-4 text-center">
                    <span
                      className={clsx(
                        ...
                        u.isActive ? "text-field-400" : "text-ember-400",
                      )}
                    >
                    ...
                    <div className="flex items-center justify-end gap-3">
                      ...
                      <button
                        ...
                        className={clsx(
                          "transition-colors",
                          u.isActive
                            ? "text-slate-500 hover:text-volt-400"
                            : "text-field-400 hover:text-field-300",
                        )}
      ...
      <CreateUserModal
        ...
        onCreate={async (body) => {
          try {
            ...
            toast.error(
              err?.data?.message || "Couldn't create user — try again",
            );
          }
        }}
      />
          ...
          <ResetPasswordForm
            ...
            onSave={async (newPassword) => {
              try {
                ...
                toast.error(
                  err?.data?.message || "Couldn't reset password — try again",
                );
              }
            }}
          />
...
const CreateUserModal: React.FC<{
  ...
  const { data: franchises } = franchiseApi.useGetFranchisesQuery({
    isActive: true,
  });
          ...
          >
            ...
            {(franchises ?? []).map((f) => (
              <option key={f.id} value={f.id}>
                {f.academyName ? `${f.academyName} — ${f.name}` : f.name}
              </option>
            ))}
          </select>
        </div>
