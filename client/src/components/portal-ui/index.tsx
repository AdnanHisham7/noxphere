  ...
  return (
    <div className="nox-card p-5">
      <div className="flex items-center justify-between">
        <span className="text-[11px] font-mono uppercase tracking-wide text-nox-low">
          {label}
        </span>
        {icon && <span className={accentText}>{icon}</span>}
      </div>
      <div className={`mt-2 font-orbital text-2xl font-semibold ${accentText}`}>
        {value}
      </div>
    </div>
  );
...
export const NoxSkeleton: React.FC<{ className?: string }> = ({
  className = "",
}) => (
  <div className={`animate-pulse rounded-lg bg-white/[0.06] ${className}`} />
);

export const NoxEmptyState: React.FC<{
  title: string;
  body?: string;
  icon?: React.ReactNode;
}> = ({ title, body, icon }) => (
  <div className="nox-card flex flex-col items-center text-center px-6 py-14">
    ...
    <h3 className="font-orbital text-base font-medium text-nox-high">
      {title}
    </h3>
...
export const NoxPageHeader: React.FC<{
  eyebrow?: string;
  title: string;
  subtitle?: string;
  ...
  <div className="mb-8">
    ...
    <h1 className="mt-2 font-orbital text-2xl md:text-3xl font-semibold text-nox-high">
      {title}
    </h1>
